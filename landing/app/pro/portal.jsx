 ...
 */
/** Les onglets, nommés selon le rôle : un grossiste publie des lots, pas des offres. */
function tabsFor(voc) {
  return [
    { key: 'dashboard', label: 'Tableau de bord', icon: '▦' },
    { key: 'orders', label: 'Commandes', icon: '📦' },
    { key: 'documents', label: 'Tickets & factures', icon: '🧾' },
    { key: 'offers', label: voc.offersTitle, icon: '％' },
    { key: 'profile', label: voc.profileTab, icon: '🏪' },
    // Le dernier, parce qu'on n'y va qu'en cas de problème — mais TOUJOURS
    // présent : un professionnel bloqué sans interlocuteur abandonne la
    // plateforme, il n'ouvre pas un ticket ailleurs.
...
/**
 * CE QUI CHANGE ENTRE LES DEUX ESPACES — et rien d'autre.
 *
 * Le grossiste et le commerçant partagent tout : commandes, documents, offres,
 * fiche du commerce. Leurs différences tiennent au vocabulaire et à UNE règle
 * de parcours — le schéma remis par le client fait diverger leurs statuts à
 * l'avant-dernière étape : un grossiste EXPÉDIE, un commerçant remet en main
 * propre. Les confondre ferait mentir l'un des deux écrans.
 *
 * Tout est ici pour que la prochaine différence s'ajoute à un seul endroit.
 */
const ROLES = {
  commercant: {
    space: 'Espace professionnel',
    entity: 'Le Comptoir du Midi',
    offersTitle: 'Offres',
    offersHint: 'Vos ventes flash, telles que vos clients les voient.',
    profileTab: 'Mon commerce',
    profileTitle: 'Mon commerce',
    buyerLabel: 'client',
    /** Le commerçant remet en main propre ou confie au livreur. */
    actions: {},
    /** Commission B2C — CDC §5.4, arbitrée à 5 %. */
    commissionPct: COMMISSION_PCT,
  },
  grossiste: {
    space: 'Espace grossiste',
    entity: 'Occitanie Fruits & Légumes',
    offersTitle: 'Lots',
    offersHint: 'Vos lots B2B, tels que les commerçants les voient.',
    profileTab: 'Mon entreprise',
    profileTitle: 'Mon entreprise',
    buyerLabel: 'commerçant',
    /**
     * Le grossiste EXPÉDIE. Le schéma du client fait diverger les deux
     * parcours exactement là : « Expédiée → Livrée » au lieu de « Remise
     * client / livreur ». Les confondre ferait mentir l'un des deux écrans.
     */
    actions: { en_livraison: 'Expédier', recuperee: 'Remise au transporteur' },
    /**
     * Commission B2B — VOLONTAIREMENT ABSENTE.
     *
     * Le §10.2 du CDC est explicite : « le taux de 5 % B2C ne doit pas être
     * copié automatiquement sur le B2B […] aucun taux B2B ne doit être codé en
     * dur ». Les paniers et les marges n'ont aucune commune mesure. Afficher
     * 5 % ici serait inventer le modèle économique du client sur son propre
     * espace — l'écran dit donc qu'il ne le connaît pas.
     */
    commissionPct: null,
  },
};

export default function ProPortal({ role = 'commercant' }) {
  const voc = ROLES[role] ?? ROLES.commercant;
  const TABS = tabsFor(voc);
      ...
      <main className="tkpro-main">
                    ...
                    <div className="foot">
                      ...
                      <div className="actions">
                        {gestes.map((geste) => (
                          ...
                          >
                            {voc.actions[geste.target] ??
                              ACTION_LABEL[geste.target] ??
                              ORDER_STATUS_LABEL[geste.target]}
                          </button>
                        ))}
                      </div>
                    </div>
        ...
        {tab === 'offers' ? (
          <>
            ...
            <p className="tkpro-note">
              {voc.offersHint} La remise s’applique à chaque vente conclue.{' '}
              {voc.commissionPct !== null
                ? `TK LINK prélève ${voc.commissionPct} % sur chaque vente (CDC §5.4).`
                : 'La commission applicable aux ventes B2B n’est pas encore arrêtée : elle vous sera communiquée avant toute facturation (CDC §10.2).'}
            </p>
          </>
        ) : null}
        {tab === 'profile' ? <MerchantProfile voc={voc} onSave={notify} /> : null}
        {tab === 'contact' ? <ContactPanel voc={voc} onSend={notify} /> : null}
      </main>
 ...
 */
function MerchantProfile({ voc, onSave }) {
...
/** Ce dont on veut parler. Router la demande évite le « bonjour, j'ai un souci ». */
const SUJETS = [
  { key: 'commande', label: 'Un problème sur une commande' },
  { key: 'paiement', label: 'Paiement ou versement' },
  { key: 'offre', label: 'Publication d’une offre' },
  { key: 'compte', label: 'Mon compte et mes documents' },
  { key: 'autre', label: 'Autre sujet' },
];

/** Le fil déjà ouvert avec l'administration — démonstration. */
const FIL = [
  {
    id: 'm1',
    from: 'equipe',
    author: 'Équipe TK LINK',
    at: 'Hier · 16 h 12',
    body: 'Bonjour, votre compte est validé et vos cinq premières opérations sont créditées. Bonne mise en ligne !',
  },
];

/**
 * CONTACT — la porte de sortie quand quelque chose coince.
 *
 * Demandé par la conduite de projet : « ajoute un onglet contact, pour que les
 * commerçants puissent échanger avec l'équipe d'administration en cas de
 * soucis ». Un professionnel bloqué sans interlocuteur n'ouvre pas un ticket
 * ailleurs : il arrête d'utiliser la plateforme.
 *
 * Le sujet est demandé AVANT le message, et ce n'est pas une formalité : c'est
 * lui qui route la demande vers la bonne personne. Sans lui, tout arrive dans
 * la même boîte sous la forme « bonjour, j'ai un souci ».
 */
function ContactPanel({ voc, onSend }) {
  const [sujet, setSujet] = useState(SUJETS[0].key);
  ...
  const envoyer = (event) => {
    ...
    setFil((f) => [
      ...f,
      { id: `m_${Date.now()}`, from: 'moi', author: voc.entity, at: 'À l’instant', body: texte },
    ]);
    setMessage('');
    onSend('Message envoyé — réponse sous 24 h ouvrées.');
  };
  ...
  return (
    <>
      <div className="tkpro-card-head">
        <h2>Contact</h2>
        <span className="tkpro-note-inline">Réponse sous 24 h ouvrées</span>
      </div>

      <div className="tkpro-card">
        <div className="tkpro-fil">
          {fil.map((m) => (
            <div className={m.from === 'moi' ? 'tkpro-msg moi' : 'tkpro-msg'} key={m.id}>
        ...
        <form className="tkpro-contact-form" onSubmit={envoyer}>
          ...
          <label className="tkpro-input wide">
            ...
            <textarea
              ...
              placeholder="Décrivez le problème — numéro de commande, date, montant si besoin."
            />
          </label>

          <div className="tkpro-profile-foot">
            {/* Désactivé tant que le message est vide : un bouton qui ne fait
                rien quand on le presse se lit comme une panne. */}
            <button type="submit" className="tkpro-btn primary" disabled={!message.trim()}>
              Envoyer
            </button>
            <span className="tkpro-note-inline">
              Démonstration : le message reste sur cette page, aucun serveur ne le reçoit.
            </span>
          </div>
        </form>
      </div>
      ...
      <p className="tkpro-note">
        Pour une urgence bloquante — un paiement non reçu, une offre en ligne à retirer — appelez le
        05 82 95 12 40, du lundi au vendredi de 9 h à 18 h.
      </p>
    </>
  );
}
